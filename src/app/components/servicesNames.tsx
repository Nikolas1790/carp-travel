...
const ServicesNames: FC<ServicesNamesProps>= ({ setActiveIndex, activeIndex }) => {
  return ( 
    <ul className=" absolute z-10 top-1/2 left-1/2 transform -translate-x-3/4 -translate-y-[-29%]  flex flex-col gap-4  
        md:col-start-2 md:row-start-2 md:-translate-x-[-55%] md:-translate-y-[90%]
      ...
      {Slides.map(({ id, title }, index) => (
        <li key={id}>
          <button 
            ...
            className={clsx(
              'flex items-center text-[20px] leading-[17px] uppercase w-[190px] text-left md:text-[22px] md:w-[254px] xl:text-[28px] xl:leading-[24px]',
